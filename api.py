...
class BumbleAPI:
    session: str
    myID: str = dataclasses.field(init = False)

    def __post_init__(self):
        self.myID = self.getMyUserID()

    def __signRequest(self, body) -> str:
    ...
    def getUser(self, userID: str) -> User:
        ...
        return from_dict(data_class = User, data = json)

    def getChat(self, userID: str) -> Chat:
        ...
        return from_dict(data_class = Chat, data = resp.json()["body"][0]["client_open_chat"])

    def sendMessage(self, toID: str, message: str, messageID: int = 2147483647) -> requests.Response:
    ...
    # max is 20 users in match queue
    def getMatchQueue(self, numberUsers: int = 10) -> List[SearchResult]:
        ...
        return [from_dict(data_class = SearchResult, data = elem) for elem in resp.json()["body"][0]["client_encounters"]["results"]]

    def getConversationQueue(self, offset = 0, preferred_count = 10, section = 2) -> List[User]:
        conversationQueue = []
        ...
        try:
            ...
            conversationQueue.extend([from_dict(data_class = User, data = elem) for elem in resp.json()["body"][0]["client_user_list"]["section"][-1]["users"]])
            ...
            conversationQueue = conversationQueue + moreConversations
        except:
            #print("End reached")
            pass
    ...
    def voteUser(self, userID: str, vote: int) -> requests.Response:
    ...
    def dislikeUser(self, userID: str) -> None:
        self.voteUser(userID, 3)

    def likeUser(self, userID: str) -> None:
